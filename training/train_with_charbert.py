...
def run(rank, n_gpus, hps):
    ...
    if "use_wd" in hps.model.keys() and hps.model.use_wd:
        net_wd = WavLMDiscriminator(
            hps.model.slm_hidden, hps.model.slm_nlayers, hps.model.slm_initial_channel
        ).cuda(rank)
    ...
    #    print("Freezed BERT text encoder! Only projection will train...")
        for param in net_g.enc_p.encoder.parameters():
            param.requires_grad = False
        for layer in net_g.enc_p.encoder.encoder.layer[-1:]:
            for param in layer.parameters():
                param.requires_grad = True
        print("Freezed BERT text encoder except last 1 layers! Projection and last 2 layers will train...")
    ...
    except IndexError:  # no checkpoint found
        epoch_str = 1
        global_step = 0
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            train_and_evaluate(
                ...
                epoch,
                hps,
                [net_g, net_d, net_dur_disc, wl, net_wd],
    ...
    for batch_idx, (
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                if net_dur_disc is not None:
                    loss_dur_gen, losses_dur_gen = generator_loss(
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                scalar_dict.update(
                    {"loss/d/tprls": loss_disc_tprls, "loss/g/tprls": loss_gen_tprls}
                )
